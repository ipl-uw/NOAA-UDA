# https://github.com/jindongwang/transferlearning/tree/master/code/DeepDA/loss_funcs
...
class DynamicLMMDLoss(MMDLoss, LambdaSheduler):
    def __init__(self, num_class, kernel_type='rbf', kernel_mul=2.0, kernel_num=5, fix_sigma=None,
                 gamma=1.0, max_iter=1000, **kwargs):
        '''
        Local MMD
        '''
        super(DynamicLMMDLoss, self).__init__(kernel_type, kernel_mul, kernel_num, fix_sigma, **kwargs)
    ...
    def cal_weight(self, source_label, target_logits):
        ...
        # loose the cretiria
        top_2_idx = torch.argsort(target_logits)[:,-2]
        top_2_label = top_2_idx.cpu().numpy()
        ...
        set_t_top2 = set(top_2_label)
        count = 0
        for i in range(self.num_class):  # (B, C)
            if (i in set_s and i in set_t) or (i in set_s and i in set_t_top2):
